                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    // your code!
    if (singleQueue.empty() && mergeQueue.empty()) return smallest;
    else if (singleQueue.empty() && (!mergeQueue.empty())){
        // Frequency f(mergeQueue.front()->freq.getCharacter(), mergeQueue.front()->freq.getFrequency());
        smallest = mergeQueue.front();
        mergeQueue.pop();
        return smallest;
    }
    else if (mergeQueue.empty() && (!singleQueue.empty())) {
        // Frequency f(singleQueue.front()->freq.getCharacter(), singleQueue.front()->freq.getFrequency());
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        // Frequency parent_freq(left->freq.getCharacter() + right->freq.getCharacter() ,left->freq.getFrequency() + right->freq.getFrequency());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
        bool dir_choice = bfile.getNextBit();
        if (dir_choice == 0) current = current->left;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    bool bit_read = bfile.getNextBit();
    if (bit_read == 1){
        char letter = bfile.getNextByte();
        Frequency freq(letter, 0);
        TreeNode* leaf = new TreeNode(freq);
        return leaf;
    }else{
        Frequency freq(0);
        TreeNode* internal = new TreeNode(freq);
