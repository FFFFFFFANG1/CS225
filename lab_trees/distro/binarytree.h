...
class BinaryTree
{
  ...
  public:
    ...
    // T maxdata(const Node* root) const;
    // T mindata(const Node* root) const;
  ...
  private:
    Node* root;
    int sumDistances(Node* subRoot, int & sum, int distance) const;
    void printPaths(vector<vector<T> > &paths,const Node* subRoot, vector<T> apath) const;
    void isOrdered(const Node* subRoot, vector<T>& lis) const;
