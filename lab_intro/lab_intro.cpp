...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++){
    for (unsigned y = 0; y < image.height(); y++){
      HSLAPixel *pixel = image.getPixel(x, y);
      double distance;
      distance = sqrt((x-centerX)*(x-centerX) + (y-centerY)*(y-centerY)); //this is the distance of a point to the center of the image
      if (distance > 200) {
        pixel->l = 0; //if the distance is over 200 pixels away, the luminance will always be 0
      }
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel *pixel = image.getPixel(x, y);
      double middle_point = pixel->h;
      if (middle_point >= 113.5 && middle_point < 293.5) { //find the middle point
        pixel->h = 216;
      }
      else {
        pixel->h = 11;
