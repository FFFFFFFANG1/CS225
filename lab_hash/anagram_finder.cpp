...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    Dict<char, int> wordDict(word.length());
    Dict<char, int> testDict(test.length());
    for (size_t i = 0; i < word.length(); i++) {
        wordDict[word[i]]++;
        testDict[test[i]]++;
    }
    for (size_t i = 0; i < word.length(); i++) {
        if (!testDict.keyExists(word[i])) return false;
        if (wordDict[word[i]] != testDict[word[i]]) return false;
    }
