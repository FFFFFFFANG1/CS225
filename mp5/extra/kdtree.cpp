                                ...
                                const Point<Dim>& potential) const
{
    ...
    if (pot_dis == curr_dis) return potential < currentBest;
    return pot_dis < curr_dis;
}


/** * Perform a quickselect algorithm that given a vector of points, return the middle large member under current dimension. **/
template <int Dim>
Point<Dim> KDTree<Dim>::quickselect(vector<Point<Dim>>& newPoints, int left, int right, int k, int curDim)
{
    ...
    if (k == pivotIdx) return newPoints[k];
    else if (k < pivotIdx) return quickselect(newPoints, left, pivotIdx - 1, k, curDim);
    else return quickselect(newPoints, pivotIdx + 1, right, k, curDim);
}
...
typename KDTree<Dim>::KDTreeNode* KDTree<Dim>::build_KD(vector<Point<Dim>>& KD_sorted, int left, int right)
{
  ...
  root->left = build_KD(KD_sorted, left, mid - 1);
  root->right = build_KD(KD_sorted, mid + 1, right);
  return root;
}
...
KDTree<Dim>::KDTree(const vector<Point<Dim>>& newPoints)
{
    ...
    vector<Point<Dim>> point_list = newPoints;
 ...
 * If so, it calls findNearestNeighbor recursively to check the other side. **/
template <int Dim>
