...
bool StickerSheet::translate(unsigned index, unsigned x, unsigned y){
    if (index >= total || x + X_[index] >= base_pic.width() || y + Y_[index] >= base_pic.height()) return false;
    int judge = 1;
...
void StickerSheet::removeSticker(unsigned index){
    ...
    stic_arr[index] = NULL;
    for (unsigned i = index; i < total -1 ; i++){
        stic_arr[i] = stic_arr[i+1];
        X_[i] = X_[i+1];
        Y_[i] = Y_[i+1];
    }
    
    X_[total - 1] = Y_[total - 1] = 0;
    stic_arr[total - 1] = NULL;
    used--;
    return;
}

Image* StickerSheet::getStiker(unsigned index) const{
...
Image StickerSheet::render() const{
    Image pic =  Image();
    pic.resize(base_pic.width(), base_pic.height());
    pic = base_pic;
    /*resize check*/
    for (unsigned k = 0; k < used; k ++){    
        if (stic_arr[k]->width() + X_[k] > pic.width()) pic.resize(base_pic.width() + stic_arr[k]->width(), base_pic.height());
        if (stic_arr[k]->height() + Y_[k] > pic.height()) pic.resize(base_pic.width(), base_pic.height() + stic_arr[k]->height());
