...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    BTreeNode* child = parent->children[child_idx]; //6, here child_idx should be 1
    /* The "left" node is the old child, the right child is a new node. */
    BTreeNode* new_left = child; //6
    BTreeNode* new_right = new BTreeNode(child->is_leaf, order); //blank after 2

    /* E.g.
     * | 3 | 6 | 8 |
     * Mid element is at index (3 - 1) / 2 = 1 . //6
     * Mid child (bar) is at index 4 / 2 = 2 . //between 6 and 8.
     * E.g.
     * | 2 | 4 |
     ...
     */
    size_t mid_elem_idx = (child->elements.size() - 1) / 2; //1 in this case
    size_t mid_child_idx = child->children.size() / 2; //2 in this case, bteween 6 and 8

    /* Iterator for where we want to insert the new child. */
    auto child_itr = parent->children.begin() + child_idx + 1; //8
    /* Iterator for where we want to insert the new element. */
    auto elem_itr = parent->elements.begin() + child_idx;  //begin + 1
    /* Iterator for the middle element. */
    auto mid_elem_itr = child->elements.begin() + mid_elem_idx; //6
    /* Iterator for the middle child. */
    auto mid_child_itr = child->children.begin() + mid_child_idx; //between 6 and 8
