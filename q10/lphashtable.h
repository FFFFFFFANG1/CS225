  ...
  public:
    ...
    void printTable() const 
    {
        for (unsigned idx = 0; idx < size; idx++) {
            std::cout << "idx: " << idx << " should probe: " << should_probe[idx];
            if (table[idx])
                std::cout << " Key: '" << table[idx]->first << "' Value: " <<  table[idx]->second;
            std::cout << std::endl;
        }
    }
    ...
    {
        for (unsigned idx = 0; idx < size; idx++) {
            file << "idx: " << idx << " should probe: " << should_probe[idx];
            if (table[idx])
                file << " Key: '" << table[idx]->first << "' Value: " <<  table[idx]->second;
            file << std::endl;
        }
    }
  private:
