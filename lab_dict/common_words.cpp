...
void CommonWords::init_common()
{
    ...
    for (size_t i = 0; i < file_word_maps.size(); i ++){
        for (std::pair<const string, unsigned int> & p : file_word_maps[i]) {
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    vector<string> out;
    /* Your code goes here! */
    for (std::pair<const std::string, unsigned int> p : common) { 
        if (p.second == file_word_maps.size()){
            string key = p.first;
            bool remark = true;
            for (size_t i = 0; i < file_word_maps.size(); i ++){
                if (file_word_maps[i].find(key)->second < n){
                    remark = false;
                    break;   
                }
            }
            if (remark == true) out.push_back(key);
        }
    }
    return out;
}
