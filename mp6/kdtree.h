...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIdx, int curDim);
    // int mid(int left, int right);
    Point<Dim> quickselect(vector<Point<Dim>>& newPoints, int left, int right, int k, int curDim);
    void swap(Point<Dim>& a, Point<Dim>& b);
    void KD_Vec(vector<Point<Dim>>& newPoints, vector<Point<Dim>>& KD_sorted ,int left, int right, int curDim);
    typename KDTree<Dim>::KDTreeNode* build_KD(vector<Point<Dim>>& KD_sorted, int left, int right);
    void in_order_push(KDTreeNode* root, vector<Point<Dim>>& KD_sorted);
    void clear(KDTreeNode* root);
    void findNearestNeighbor(const Point<Dim>& query, Point<Dim>& cur_best, KDTreeNode* subroot, int curDim) const;
};
