...
SkipNode * SkipList::findRHelper(int key, int level, SkipNode * curr)
{
    ...
    if (nextKey == key){
        return ret = curr->nodePointers[level].next;
    }
...
SkipNode * SkipList::findI(int key)
{
    ...
    while (traverse->nodePointers[0].next != tail && level >= 0)
    {
        if (traverse->nodePointers[level].next == NULL) return NULL;
        int nextKey = traverse->nodePointers[level].next->key;

        if (nextKey == key)
        {
            return retNode = traverse->nodePointers[level].next;
        }
        else if (key < nextKey)
        {
            level--;
